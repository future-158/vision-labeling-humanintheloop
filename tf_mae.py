# -*- coding: utf-8 -*-
"""buzzni-masked-autoencoder
Automatically generated by Colaboratory.
Original file is located at
    https://colab.research.google.com/drive/1zalM3Av74l7et2LrCNNnv0rnsgI66fvA
"""
...
TRAIN = False
TEST_DIR = Path('/content/drive/MyDrive/dataset/computer_vision_task/test')
TRAIN_DIR = Path('/content/drive/MyDrive/dataset/computer_vision_task/train')
...
#     (x_train[4000:5000], y_train[4000:5000]),
# )
...
AUTO = tf.data.AUTOTUNE
train_files = list(TEST_DIR.glob('**/*.jpg'))
test_files = list(TRAIN_DIR.glob('**/*.jpg'))
...
# path = "/content/drive/MyDrive/dataset/computer_vision_task/saved_data"
...
total_steps = int(( TRAIN_SIZE / BATCH_SIZE) * PRETRAINING_EPOCHS)
...
mae_ckpt_path = 'drive/MyDrive/buzzni/model/mae/checkpoint'
encoder_ckpt_path = 'drive/MyDrive/buzzni/model/encoder/checkpoint'
...
train_callbacks = [TrainMonitor(epoch_interval=5), ckpt_callback,
                   es_callback]
...
if TRAIN:
    history = mae_model.fit(
        train_ds, epochs=PRETRAINING_EPOCHS, validation_data=test_ds, callbacks=train_callbacks,
    )
    ...
    mae_model.encoder.save_weights(encoder_ckpt_path)

# new_model = tf.keras.models.load_model(encoder_model_path)
...
encoder = create_encoder()
encoder.load_weights(encoder_ckpt_path)
...
def prepare_data(dataset, is_train=True):
    if is_train:
...
val_ds = prepare_data(val_ds, is_train=False)
test_ds = prepare_data(test_ds, is_train=False)
...
def calc_buzzni(y_test, y_proba, k = 5):
    y_pred = np.argmax(y_proba, axis=-1)
    num_samples = len(y_test)
    FN =  FP = num_samples - top_k_accuracy_score(y_test, y_proba, k = k, normalize=False)
    TP_at_1 = top_k_accuracy_score(y_test, y_proba, k=1, normalize=False)
    TP_at_k = ( top_k_accuracy_score(y_test, y_proba, k = k, normalize=False) - TP_at_1) / k
    TP =  TP_at_1 + int(TP_at_k)
    Precision = TP/ (TP + FP)
    Recall = TP / (TP + FN)
...
def calc_buzzni_metric(y_test, y_proba, k = 5):
    y_pred = tf.argmax(y_proba, axis=-1)
    y_test_flat = tf.cast(tf.reshape(y_test, [-1]), tf.int32)
    num_samples = tf.cast(len(y_test), tf.float32)
    TP_at_1 = tf.math.reduce_sum(
        tf.keras.metrics.top_k_categorical_accuracy(
            tf.one_hot(y_test_flat, depth=NUM_CLASSES),
            y_proba,
            k=1
            ), axis=None
    )

    TP_at_k = tf.math.reduce_sum(
        tf.keras.metrics.top_k_categorical_accuracy(
            tf.one_hot(y_test_flat, depth=NUM_CLASSES),
            y_proba,
            k=k
...
# find hierachi. using confusionmatix simalaraity
# predict group, catalog. 
# if catalog match, + tp
# else if group match, + tp / normalized by group difficulty
# both fail. add + fp
# find precision score.
# pre trainined resent with cifar. find similiary score with test set.(find hierachi.)

LINEAR_PROBE_EPOCHS = 50
linear_prob_lr = 0.1
warm_epoch_percentage = 0.1
steps = int((TRAIN_SIZE // BATCH_SIZE) * LINEAR_PROBE_EPOCHS)
...
downstream_model.compile(
    optimizer=optimizer, loss="sparse_categorical_crossentropy", metrics=["accuracy", calc_buzzni_metric]
)
downstream_model.fit(train_ds, validation_data=test_ds, epochs=LINEAR_PROBE_EPOCHS)

y_proba = downstream_model.predict(test_ds)

y_test = np.array(list(test_ds.unbatch().map(lambda x,y: y).as_numpy_iterator()))

tf.math.reduce_mean(
        tf.keras.metrics.top_k_categorical_accuracy(
            tf.one_hot(y_test, depth=50),
            y_proba,
            k=30
            ), axis=None
    )

calc_buzzni_metric(y_test, y_proba, k=45)
